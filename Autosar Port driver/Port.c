	...
	if (NULL_PTR == ConfigPtr)
	{
...
#endif  
        {
            ...
            for(PinID = 0; PinID < PORT_CONFIGURED_PINS; PinID++)  /* Requirment PORT041 */
            {
             ...
             /* Analog or Digital  */
              if(Port_PortPins[PinID].Mode == ADC_MODE) /* analog pin */
              {
              ...
              *(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_CTL_REG_OFFSET) &= ~(0x0000000F << pin_num * 4 );  
              ...
              if(Port_PortPins[PinID].Mode == PORT_PIN_MODE_DIO /* 0x00 */ || Port_PortPins[PinID].Mode == ADC_MODE  /* 0x0A */ )
              { /* GPIO */ 
                /* case PORT_PIN_MODE_DIO & case PORT_PIN_MODE_AINx & PORT_PIN_MODE_Cx */
...
void Port_SetPinDirection( Port_PinType Pin,  Port_PinDirectionType Direction)
{
	...
	else
	{
        ...
        /* Pin direction changable checking */
        if( Port_PortPins[Pin].Pin_Direction_changeable == STD_OFF)
        {
...
void Port_SetPinMode(Port_PinType Pin, Port_PinModeType Mode )
{
    ...
    else
    {
     ...
     /* Pin direction changable checking */
    if( Port_PortPins[Pin].Pin_Mode_changeable == STD_OFF)
    {
    ...
    /* Invaild mode checking */
    if(Mode > 0x0A && Mode!= 0x0E)  //Invalid Mode
    {
            Det_ReportError(PORT_MODULE_ID, PORT_INSTANCE_ID,
  ...
  if(FALSE == error)
  {
    ...
    /* Pin parameter must be an index 0 - 38 */
        ...
        /* Mode can be changed */
        /* Analog or Digital  */
        if(Port_PortPins[Pin].Mode == ADC_MODE ) /* ADC */
        {
          /* SET the corresponding bit in the GPIOAMSEL register to Enable analog functionality on this pin */
          SET_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_ANALOG_MODE_SEL_REG_OFFSET) , pin_num );     
	...
	*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_CTL_REG_OFFSET) &= ~(0x0000000F << pin_num * 4 );  
	...
	if(Port_PortPins[Pin].Mode == PORT_PIN_MODE_DIO /* 0x00 */ || Port_PortPins[Pin].Mode == ADC_MODE  /* 0x0A */ )
	{ /* GPIO */ 
	  /* case PORT_PIN_MODE_DIO & case PORT_PIN_MODE_AINx & PORT_PIN_MODE_Cx */
	  /* Disable Alternative function for this pin by clear the corresponding bit in GPIOAFSEL register */
	  CLEAR_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_ALT_FUNC_REG_OFFSET) ,pin_num );    
	...
	{ /* Alternative fucntion */
	  ...
	  SET_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_ALT_FUNC_REG_OFFSET) ,pin_num );
	  /* set the PMCx bits with the corresponding funtion mode number */
	  *(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_CTL_REG_OFFSET) |= (Port_PortPins[Pin].Mode << pin_num * 4 ); 
	}
...
void Port_RefreshPortDirection(void)
{
  ...
  if(FALSE == error)
  {
      ...
      for(PinID = 0; PinID < PORT_CONFIGURED_PINS; PinID++)  /* Requirment PORT041 */
      {
        ...
        if(Port_PortPins[PinID].Pin_Direction_changeable == STD_ON) /* Requirment PORT061 */
        {
               /* output or input */
           if(Port_PortPins[PinID].Direction == PORT_PIN_OUT) /* OUTPUT*/
           {
        ...
        else /* Direction is not changable  */
        {
          /* No action required  */
        }
      }  
